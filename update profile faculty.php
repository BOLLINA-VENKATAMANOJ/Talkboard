...
if((isset($_POST['email']) && isset($_POST['pno'])))
{
	...
	$filename = $_FILES['File']['name'];
			   if($filename=="")
				{	
						$_SESSION['NOFILE']="";
						$r = mysql_query("update `faculty` SET `email`='$email',`phno`='$phno' where `usname`='$id'");
					...
					$fil=mysql_query("SELECT * FROM `faculty` WHERE photo='$filename'");
						...
						{			
								...
								{	
									...
									$r = mysql_query("update `faculty` SET `email`='$email',`phno`='$phno',`photo`='$filename' where `usname`='$id'");
...
<html class="no-js">
    <head>
        <meta charset="utf-8">
        <title>Update profile faculty</title>
	...
	<div class="main-container fadeIn animated">
		...
		<div class="container">
			...
			<?php
				$id=$_SESSION['flogin'];
				$f=mysql_query("select * from `faculty` where `usname`='$id'");
				$n=mysql_num_rows($f);
				if($n >= 1)
				{
			...
			?>
					...
					<h2 class="title-style-2">FACULTY PROFILE UPDATE<span class="title-under"></span></h2><br>

					<form method="POST" action="" enctype="multipart/form-data" role="form">
						
						<div class="form-group col-md-6">
						<div class="form-group">
                            <input type="name" value="<?php echo $fetch['usname']?>"class="form-control" placeholder="User name( College Id )*" disabled="">
                        </div>
						<div class="form-group">
