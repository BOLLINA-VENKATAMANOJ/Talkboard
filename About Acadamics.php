...
<div id="section">
	<script>
		...
		<?php
			if(isset($_POST['post']))
			{
					$p=mysql_real_escape_string($_POST['post']);
					mysql_query("insert into `aboutacadamics` (`question`,`ID`) values ('$p','$id')");
			}
		
		?>
			<legend style="color:darkblue;padding:2px;">About Acadamics</</legend>
			<?php
				$r=mysql_query("select * from `aboutacadamics` where 1");
				$m=mysql_num_rows($r);
				while($m=mysql_fetch_array($r))
				{
			?>
				
				<tr><td><span style="color:darkorchid;"><?php echo $m['ID']?></span><br><a href="answeracadamics.php?ID=<?php echo $m['s.no']?>" ><?php echo $m['question']?></a></td></tr>
	...
	<form method="POST" action="" enctype="multipart/form-data" role="form">
			...
			"
			value="Post A question" onclick="addRow();" /><br/>
</form>
