...
if((isset($_POST['new']) && isset($_POST['cnew']) && isset($_POST['current'])))
{
	...
	$r=mysql_query("select * from `faculty` where `pwd`='$cpwd'");
	$c=mysql_num_rows($r);
	if($c >= 1)
	{
		...
		if("$new==$cnew")
		{
				$r = mysql_query("update `faculty` set `pwd`='$new' where `usname`='$id'");
				$_SESSION['SUCC']="";
		}
		else
		{
...
<form method="POST" action="" class="updat" enctype="multipart/form-data" role="form">
  ...
  <input class="nameu" type="text" name="current" placeholder="Enter current password"/>
