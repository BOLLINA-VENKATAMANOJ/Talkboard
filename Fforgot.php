...
if(isset($_POST['fname']) && isset($_POST['pwd']) && isset($_POST['cpwd']) && isset($_POST['seq']) && isset($_POST['answer']))
{
	...
	if($fpwd == $fcpwd){
		$sql  = mysql_query("select * from faculty  where `usname`='$username' AND `seq`='$qname' AND `seqans`='$seqans'");
		$result = mysql_num_rows($sql);
	if ($result >= 1) {
		$sql = mysql_query("UPDATE faculty SET pwd = '$fpwd' WHERE usname ='$username'");
		$_SESSION['UPDATED']="";
	}
...
<body>
	...
	{
		echo "<center><h3 style=color:green>Password Updated Successfully</h3></center>";
...
<div class="containerlog">
	<section id="content">
		<form action="" method ="POST">
			...
			<div>
				<input type="password" name="cpwd" placeholder="Confirm Password" required="" id="password" />
			</div>
			<div>
