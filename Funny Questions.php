	...
	<script>
		...
		<?php
			if(isset($_POST['post']))
			{
					...
					mysql_query("insert into `funny` (`question`,`ID`) values ('$p','$id')");
			}
		
		?>
			<legend style="color:darkblue;padding:2px;">Funny Questions</</legend>
			<?php
				$r=mysql_query("select * from `funny` where 1");
				$m=mysql_num_rows($r);
				while($m=mysql_fetch_array($r))
				{
			?>
				
				<tr><td><span style="color:darkorchid;"><?php echo $m['ID']?></span><br><a href="answerfunny.php?ID=<?php echo $m['s.no']?>" ><?php echo $m['question']?></a></td></tr>
