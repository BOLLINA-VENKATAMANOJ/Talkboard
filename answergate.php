...
<div id="section">
	<script>
		...
		<?php
			...
			$f=mysql_query("select * from `gate` where `s.no`='$sno'");
			$co=mysql_num_rows($f);
			if($co >= 1)
			{
		...
		<legend style="color:darkblue;padding:2px;"><?php echo $re['question']?></legend>
				<?php
					$re=mysql_query("select * from `answergate` where `question`='$q'");
	...
	<?php
			if(isset($_POST['post']))
			{
				...
				$re=mysql_query("insert into `answergate` (`question`,`ID`,`answer`,`flag`) VALUES ('$q','$id','$post','1')");
			}
			else{
			$re=mysql_query("insert into `answergate` (`question`,`ID`,`answer`) VALUES ('$q','$id','$post')");
