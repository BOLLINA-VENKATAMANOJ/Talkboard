	...
	<script>
		...
		<?php
			if(isset($_POST['post']))
			{
					...
					mysql_query("insert into `technical` (`question`,`ID`) values ('$p','$id')");
			}
		
		?>
			<legend style="color:darkblue;padding:2px;">Technical Questions</</legend>
			<?php
				$r=mysql_query("select * from `technical` where 1");
				$m=mysql_num_rows($r);
				while($m=mysql_fetch_array($r))
				{
			?>
				
				<tr><td><span style="color:darkorchid;"><?php echo $m['ID']?></span><br><a href="answertechnical.php?ID=<?php echo $m['s.no']?>" ><?php echo $m['question']?></a></td></tr>
